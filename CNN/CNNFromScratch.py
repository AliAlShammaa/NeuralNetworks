...
class ConvPoolLayer:
    ...
    def __init__(
        ...
        self.ConvolutionalLayerSize = (
            int(PrevlayerSize[0] - KernelSize[0] + 1),
            int(PrevlayerSize[1] - KernelSize[1] + 1),
        ...
        # self.weights = np.random.normal( loc=1.0, scale=0.2 , size=(p[0], p[1], p[2], p[3])  ) * 0.01
        # self.biases =  np.random.normal( loc=1.0, scale=0.2 , size=(len(PoolingMapsFuncs),1) ) * 0.01
