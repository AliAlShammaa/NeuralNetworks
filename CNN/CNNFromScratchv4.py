...
class ConvPoolLayer:
    ...
    def poolingMap(self, batchActivations):
        ...
        self.delAdelA = ( np.kron(stridedMaxed , np.ones( (poolX , poolY,1,1) ) )  == batchActivations ) * 1 
    ...
    def backprop(
        ...
        if self.PoolingBool == 1:
            ...
            if len(deeperDeltas.shape) == 2:
                ...
                self.errors = deltasTimesWeightsAugmented * o
...
def eval(testImages, batchSize, layers):  ## measure accuracy
    ...
    for interval in range(0, len(testImages), batchSize):
        ...
        answers = [np.argmax(x[:, q]) * 2 + 1 for q in range(len(section))]
